...
def build_groups_from_tasks(tasks150: Sequence[str], weights_classes27: Sequence[str]) -> MappingResult:
    """
    Mapping from ECGFounder tasks.txt (PTB-XL-like statement strings)
    to PhysioNet 2021 scored classes (exactly the weights.csv columns).

    Principles:
    - Only map to keys that exist in weights_classes21.
    - Prefer exact statement matches.
    """
    tasks = list(tasks150)
    exact_index = _build_exact_index(tasks)

    # --- desired mapping in task-space (statement strings) ---
