...
JEPA_LEADS = [0, 1, 6, 7, 8, 9, 10, 11]  # I, II, V1-V6
...
class EmoryH5_JEPA(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = np.asarray(self.X[idx], dtype=np.float32)  # expected raw: (12, 1000)
        ...
        if x.shape[1] != EXPECTED_INPUT_LEN:
            raise ValueError(
                f"Expected raw HEEDB length {EXPECTED_INPUT_LEN}, "
        ...
        # JEPA preprocessing: bandpass 0.67-40 Hz
        x = filtfilt(B_BANDPASS, A_BANDPASS, x, axis=-1)

        if not np.isfinite(x).all():
            x = np.nan_to_num(x, nan=0.0, posinf=0.0, neginf=0.0)

        # Select JEPA leads: I, II, V1-V6
        x = x[JEPA_LEADS]  
...
def estimate_pos_weight_21_fast(h5_path, map_matrix):
    print("Loading all labels into memory...")

    with h5py.File(h5_path, "r") as f:
        Y150 = f["label"][:]  # [N,150]

    print("Mapping labels to 21 classes...")

    Y21 = (map_matrix @ Y150.T > 0).T.astype(np.float32)

    print("Computing pos_weight21...")
...
def main():
    ...
    if cfg["train"].get("use_pos_weight", True):
        ...
        if pos_weight_path.exists():
            ...
            pw = estimate_pos_weight_21_fast(h5_path, full_ds.map_matrix)
